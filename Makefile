...
$(kernel_object_files): build/kernel/%.o : src/impl/kernel/%.rs
	mkdir -p $(dir $@) && \
	rustc $(patsubst build/kernel/%.o, src/impl/kernel/%.rs, $@) -C panic=abort -o $@ 
...
build-x86_64: $(kernel_object_files) $(x86_64_asm_object_files)
	mkdir -p dist/x86_64 && \
	ld -n -o dist/x86_64/kernel.bin -T targets/x86_64/linker.ld $(x86_64_asm_object_files) $(kernel_object_files) && \
